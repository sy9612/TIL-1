...
public class Sample_Code {
	...
	static final String NICKNAME = "김지홍";
		...
		try {
			...
			while (socket != null) {
				...
				try {
					...
					for (int i = 0; i < NUMBER_OF_BALLS; i++) {
						System.out.println(i + ": " + balls[i][0] + "/" + balls[i][1]);
					}
				} catch (Exception e) {
					bytes = new byte[1024];
				...
				double power = 0;
				count++;
				if(cnt == NUMBER_OF_BALLS) {
					cnt = 1;
				}
				
				if(balls[cnt][0] == -1 || balls[cnt][1] == -1) {
					cnt++;
				}
				
				double dx = balls[cnt][0] - balls[0][0];
				System.out.println(dx);
				double dy = balls[cnt][1] - balls[0][1];
				...
				System.out.println(r);
				angle = r + ran.nextInt(2)-1;
				power = ran.nextInt(50)+50;
				if(count == 1) {
					power = 100;
				}
